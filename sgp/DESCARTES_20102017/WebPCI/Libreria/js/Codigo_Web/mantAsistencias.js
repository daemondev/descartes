            ...
            if (data.length > 0) {
                for (var i = 0; i <= data.length; i++) {
                    if (data.length == i) {
                        ...
                        strRows += "<td style='text-align:center;'>" + data[i].vc_Abreviatura + "</td>" +
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (data.length > 0) {
                for (var i = 0; i <= data.length; i++) {
                    var objCapa = {
                        idCapacitacion: data[0].in_CapaID,
                        vcEstado: data[0].vc_Estado,
                    };

                    var objSim = JSON.stringify(objCapa);
                    sessionStorage.setItem("seCapacitacion", objSim);
                }
                window.location.href = "Frm_AsistenciaAgentes.aspx";
            }
        }//Fin Success
    }); //Fin Ajax
}
